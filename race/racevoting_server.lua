...
addCommandHandler('redo',
	function( player, command, value )
		if isPlayerInACLGroup(player, g_GameOptions.admingroup) then
			if not redo then
				redo = true
				--outputChatBox('The actual map has set again by ' .. getPlayerName(player), g_Root, 0, 240, 0)
				outputChatBox ( "[SERVER] #ffffffO mapa atual foi setado para ser #00ff00reiniciado #ffffffpor "..getPlayerNametagText(player), root, 255, 99, 99, true)
				triggerClientEvent("onSetNextMapRedo",root,redo)
			else
			    for i,playersVR in ipairs(getElementsByType ("player")) do
                    setElementData( playersVR, "hasVoted", "nope" )
                end
			    redo = false
				playersVoted = 0
                --mapRestarted = true
				--outputChatBox('Restart map aborted by ' .. getPlayerName(player), g_Root, 255, 0, 0)
				outputChatBox ( "[SERVER] #ffffffO mapa atual foi setado para #ff0000NÃO #ffffffser mais reiniciado por "..getPlayerNametagText(player), root, 255, 99, 99, true)
				triggerClientEvent("onSetNextMapRedo",root,redo)
			end
	...
	function( player, command, value )
		if isPlayerInACLGroup(player, g_GameOptions.admingroup) then
			if not stateAllowsRandomMapVote() or g_CurrentRaceMode:getTimeRemaining() < 1000 then
				...
				outputChatBox ( "[SERVER] #ffffffCurrent map " ..choice[math.random( 1, #choice )].." by "..getPlayerNametagText(player), root, 255, 99, 99, true)
				startRandomMap()
			end
		end
	end
...
function addOneMoreHunter()
	local currentMap = exports.mapmanager:getRunningGamemodeMap()
	if currentMap then
		local mapInfo = getMapInfo( currentMap )
		if mapInfo then
			mapInfo.hunterCount = (tonumber(mapInfo.hunterCount) or 0) + 1
			saveMapInfoItem( currentMap, mapInfo )
		end	
	end
end

function addOneMoreToptime()
	local currentMap = exports.mapmanager:getRunningGamemodeMap()
	if currentMap then
		local mapInfo = getMapInfo( currentMap )
		if mapInfo then
			mapInfo.toptimesCount = (tonumber(mapInfo.toptimesCount) or 0) + 1
			saveMapInfoItem( currentMap, mapInfo )
		end	
	end
end
addEvent("onPlayerToptimeImprovement",true)
addEventHandler("onPlayerToptimeImprovement",getRootElement(), addOneMoreToptime)
...
function getTableName(value)
	return sqlString( 'race_mapInfo_database' )
end
...
function loadMapInfoAll()
	...
	for i,row in ipairs(rows) do
		...
		if map then
			...
			mapInfo.hunterCount = row.hunterCount
			mapInfo.toptimesCount = row.toptimesCount
		end
	end
end
...
function saveMapInfoItem( map, info )
	...
	cmd = ( 'UPDATE ' .. getTableName() .. ' SET '
					...
					.. ',hunterCount='			.. sqlInt( info.hunterCount )
					.. ',toptimesCount='		.. sqlInt( info.toptimeCount )
			.. ' WHERE '
...
addCommandHandler('nextmap',
	function( player, command, ... )
		...
		outputChatBox ( "[SERVER] #ffffffPróximo mapa setado para #ff6464" ..getMapName( g_ForcedNextMap ).." #ffffffpor "..getPlayerNametagText(player), root, 255, 99, 99, true)
		g_MapInfo.nextmap = getMapName(g_ForcedNextMap)
		triggerClientEvent("onSetNextMap",root,g_MapInfo.nextmap)
		triggerEvent("nextMapByRace",root,true)
	end
)
...
--
---------------------------------------------------------------------------
--[[addEvent("onUseranelWantSetMap",true)
addEventHandler("onUseranelWantSetMap",root,
function ( ... )
    local query = #{...}>0 and table.concat({...},' ') or nil
    if not query then
		return	
	...
	g_ForcedNextMap = map
	triggerClientEvent("onSetNextMap", getRootElement(),getMapName( g_ForcedNextMap ))
    g_MapInfo.nextmap = getMapName(g_ForcedNextMap)
	outputChatBox(getMapName(g_ForcedNextMap))
end
)]]

--=============================

addEvent('onUserpanelWantNextmap',true)
addEventHandler('onUserpanelWantNextmap',getRootElement(),
function(tableValue)
	if tableValue ~= 'Random map' then
		local resourceName = tableValue[2]
...
function UpdateNext(...) 
    if g_ForcedNextMap then
		...
		g_MapInfo.nextmap = false
	end
	return
end
addEvent("onMapStarting",true)
addEventHandler("onMapStarting",root,UpdateNext)
...
function PlayerVoted(thePlayer)
    if VoteRedo then
	    if not stateAllowsRandomMapVote() or g_CurrentRaceMode:getTimeRemaining() < 1000 then
		    outputChatBox("[SERVER] "..color.."Comando é habilitado apenas enquanto há jogo.", thePlayer, 255, 99, 99, true)
		    return
		end
        if mapRestarted then
		    if redo then
			    outputChatBox("[SERVER] "..color.."O mapa será restartado automaticamente.", thePlayer, 255, 99, 99, true)
				return
            end
            if (getElementData( thePlayer, "hasVoted" ) == "nope") then
			    setElementData( thePlayer, "hasVoted", "voted" )
                playersVoted = playersVoted + 1
                outputChatBox("[SERVER] #ffffff("..playersVoted.."/".. math.floor(getPlayerCount() /2 + 0.5) ..")"..color.." O jogador #ffffff"..getPlayerNametagText(thePlayer).." "..color.."votou para reiniciar este mapa.", root, 255, 99, 99, true)
				if playersVoted >= math.floor(getPlayerCount() /2 + 0.5) then
                    redo = true
                    outputChatBox ("[SERVER] "..color.."Votação finalizada. O mapa será restartado!", root, 255, 99, 99, true)
					triggerClientEvent("onSetNextMapRedo",root,redo)
			        triggerEvent("redoByRace",root,redo)
				end
			else
				outputChatBox("[SERVER] "..color.."Você já votou.", thePlayer, 255, 99, 99, true)
			end
        else
            outputChatBox("[SERVER] "..color.."O mapa já foi restartado.", thePlayer, 255, 99, 99, true)
        end
    else
        outputChatBox("[SERVER] "..color.."Vote redo está desligado.", thePlayer, 255, 99, 99, true)
    end
end
...
addCommandHandler("deletemap",
	function(thePlayer,command)
		local currentMap = exports.mapmanager:getRunningGamemodeMap()
		if currentMap then
			if isPlayerInACLGroup(thePlayer, g_GameOptions.admingroup) then
				local resourceState = getResourceState(currentMap)
				if resourceState == "running" then
					if stopResource(currentMap) then
						setTimer(deleteMapWithDelay,1000,1,currentMap,thePlayer)
					end
				end
			end
		end
...
function deleteMapWithDelay(mapResource,thePlayer)
	if mapResource then
		local mapName = getMapName(mapResource)
		outputChatBox ( "[SERVER] #ffffffO mapa #ff6464" ..mapName.." #fffffffoi #ff0000deletado #ffffffpor "..getPlayerNametagText(thePlayer), root, 255, 99, 99, true)
		deleteResource(mapResource)
		startRandomMap()
	end
end
