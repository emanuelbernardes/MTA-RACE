...
function cacheGameOptions()
	g_GameOptions = {}
	g_GameOptions.color					= getString('race.color',"#00aaff")
	...
	g_GameOptions.randommaps			= getBool('race.randommaps',true)
	...
	g_GameOptions.blurlevel				= getNumber('race.blur',0)
	...
	g_GameOptions.hunterminigun			= getBool('race.hunterminigun',false)
	...
	g_GameOptions.joinrandomvote		= getBool('race.joinrandomvote',false)
...
function cacheMapOptions(map)
	g_MapOptions = {}
	g_MapOptions.duration = g_GameOptions.defaultduration or map.duration and tonumber(map.duration) > 0 and map.duration*1000
	if g_MapOptions.duration > 86400000 then
		g_MapOptions.duration = 86400000
	end
	g_MapOptions.respawn = g_GameOptions.defaultrespawnmode or map.respawn
	...
	if stateAllowsTimesUp() then
		...
		for i,player in pairs(g_Players) do
			if not isPlayerFinished(player) then
				triggerClientEvent("winnerMessage",resourceRoot, "Time\'s up!", "")
			end
		end
...
function toggleServerGhostmode(player)
	...
	if g_GameOptions.ghostmode then
		outputChatBox('Ghostmode ligado por ' .. getPlayerName(player), g_Root, 0, 240, 0)
	else
		outputChatBox('Ghostmode desligado por ' .. getPlayerName(player), g_Root, 240, 0, 0)
	end
end
...
function clientWantUpdateSettings()
	if source then
		clientCall(source,'updateOptions', g_GameOptions)
	end
end
addEvent("onClientWantUpdateRaceSettings",true)
addEventHandler("onClientWantUpdateRaceSettings",getRootElement(),clientWantUpdateSettings)

-------------------------------------------

function getMapInfoForMap(resourceData)
	local map = RaceMap.load(resourceData)
	g_MapInfo = getMapInfo(resourceData)
	local info = {}
	local resource       = map.info['resname'] or getResourceName(resourceData)
	local lastTimePlayed = getRealDateTimeString(getRealTime(g_MapInfo.lastTimePlayed)) or "Never played yet"
	local playedCount    = g_MapInfo.playedCount or "0"
	local hunter         = g_MapInfo.hunterCount or "0"
	local topTimesCount  = g_MapInfo.toptimesCount or "0"
	local like, dislike  = getMapRate(resource)
	table.insert(info,{"lastTimePlayed: "..lastTimePlayed,"playedCount: "..playedCount,"hunterCount: "..hunter,"topTimesCount: "..topTimesCount,"likeCount: "..like,"dislikesCount: "..dislike})
	return info
end

addEvent("xgetMapInfo",true)
addEventHandler("xgetMapInfo",getRootElement(),
function(player)
    local currentMap = exports.mapmanager:getRunningGamemodeMap()
    local table = getMapInfoForMap(currentMap)
    triggerClientEvent(player,"getMapInfo",root,table)
end
)

function getMapInfoForRedo(arg)
    if arg then
        return mapRestarted, redo
	else
	    redo = true
...
addEvent("changeSpanwPlayer",true)
addEventHandler("changeSpanwPlayer",root,
function(player,key)
    if not stateAllowsRandomMapVote() or g_CurrentRaceMode:getTimeRemaining() < 1000 then
        ...
        local maxIndex = #g_Spawnpoints
		if #g_Spawnpoints == 0 or #g_Spawnpoints == 1 then return end
		local now = tonumber(getElementData(player,"spawnId")) or 1
        if key == "up" then
	        if now <= 1 then return end
	        now = now - 1
		    local x, y, z = unpack(g_Spawnpoints[now].position)
		    local rot = g_Spawnpoints[now].rotation
            setElementPosition(vehicle,x,y,z)
		    setElementRotation(vehicle,0,0,rot)
			triggerClientEvent(player,"updateCamChangeSpawn",root,vehicle)
	    end
	    if key == "down" then
	        if now >= maxIndex then now = maxIndex return end
	        now = now + 1
		    local x, y, z = unpack(g_Spawnpoints[now].position)
		    local rot = g_Spawnpoints[now].rotation
            setElementPosition(vehicle,x,y,z)
		...
		setElementData(player,"spawnId",now)
	end
end
)